...
class TecanAPISerial(TecanAPI):
    ...
    @classmethod
    def findSerialPumps(cls, tecan_addrs=[0], ser_baud=9600, ser_timeout=0.2, max_attempts=2):
    ...
    def __init__(self, tecan_addr, ser_port, ser_baud=9600, ser_timeout=0.1, max_attempts=5):
        super(TecanAPISerial, self).__init__(tecan_addr)

        self.id_ = str(uuid.uuid4())
        self.ser_port = ser_port
        self.ser_info = {"baud": ser_baud, "timeout": ser_timeout, "max_attempts": max_attempts}
        self._registerSer()
    ...
    def sendRcv(self, cmd):
        ...
        while attempt_num < self.ser_info["max_attempts"]:
            try:
                ...
                sleep(0.2)
        raise (TecanAPITimeout("Tecan serial communication exceeded max attempts [{0}]".format(self.ser_info["max_attempts"])))
    ...
    def _registerSer(self):
        ...
        if self.ser_port not in reg:
            ...
            reg[port]["_ser"] = serial.Serial(port=port, baudrate=reg[port]["info"]["baud"], timeout=reg[port]["info"]["timeout"])
            reg[port]["_devices"] = [self.id_]
        else:
            if len(set(self.ser_info.items()) & set(reg[port]["info"].items())) != 3:
                raise serial.SerialException(
                    "TecanAPISerial conflict: another device is already registered to {0} with different parameters".format(port)
                )
            else:
...
class TecanAPINode(TecanAPI):
    ...
    def __init__(self, tecan_addr, node_addr, response_len=20, max_attempts=5):
    ...
    def sendRcv(self, cmd):
        ...
        while attempt_num < self.max_attempts:
            ...
            if attempt_num == 1:
                ...
                frame_out = self.emitRepeat()
            url = "http://{0}/syringe?LENGTH={1}&SYRINGE={2}".format(self.node_addr, self.response_len, frame_out)
            raw_in = self._jsonFetch(url)
            frame_in = self._analyzeFrame(raw_in)
            if frame_in:
                return frame_in
            sleep(0.2 * attempt_num)
        raise (TecanAPITimeout("Tecan HTTP communication exceeded max attempts [{0}]".format(self.max_attempts)))
    ...
    def _buildFrame(self, repeat=False):
        ...
        frame_list = [self.START_BYTE, self.addr, seq_byte] + self._assembleCmd() + [self.STOP_BYTE]
    ...
    def _analyzeFrame(self, raw_packet):
        ...
        raw_frame = [data_str[i : i + 2].decode("hex") for i in range(0, len(data_str), 2)]
        try:
            # Get basic indices
            frame = raw_frame[raw_frame.index(chr(self.START_BYTE)) : raw_frame.index(chr(self.STOP_BYTE)) + 2]
            if len(frame) < 5:
                return False
        ...
        payload = {"status_byte": status_frame, "data": data}
        return payload
