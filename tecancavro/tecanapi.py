...
class TecanAPI(object):
    ...
    def _analyzeFrame(self, raw_frame: bytes) -> bool | dict:
        try:
            # Get basic indices
            frame_list: list[int] = [byte for byte in raw_frame]
            frame = frame_list[
                frame_list.index(self.START_BYTE) : frame_list.index(self.STOP_BYTE) + 2
            ]
            if len(frame) < 5:
                return False
        ...
        # Dump payload
        if data_len != 0:
            data = b"".join([chr(i).encode("utf-8") for i in frame_list[3:etx_idx]])
        else:
            data = None
    ...
    def _buildFrame(self, repeat=False):
        ...
        frame_list = (
            [self.START_BYTE, self.addr, seq_byte]
            + self._assembleCmd()
            + [self.STOP_BYTE]
        )
    ...
    def _assembleCmd(self):
        ...
        try:
            ...
            if isinstance(self._cmd, int):
                ...
                raise TypeError(
                    "TecanAPI: command {0} is neither iterable nor an int".format(
                        self._cmd
                    )
                )
