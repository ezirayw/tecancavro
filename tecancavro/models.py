...
class XCaliburD(Syringe):
    ...
    def init(self, init_force=None, direction=None, in_port=None, out_port=None):
        """
        ...
        cmd_string = "{0}{1},{2},{3}".format(
            self.__class__.DIR_DICT[direction][1], init_force, in_port, out_port
        )
    ...
    def primePort(
        self, in_port, volume_ul, speed_code=None, out_port=None, split_command=False
    ):
        """
    ...
    def changePort(self, to_port, from_port=None, direction="CW"):
        ...
        if not 0 < to_port <= self.num_ports:
            raise (
                ValueError(
                    "`to_port` [{0}] must be between 1 and `num_ports` [{1}]".format(
                        to_port, self.num_ports
                    )
                )
            )
        if not from_port:
    ...
    def movePlungerAbs(self, abs_position):
        ...
        if self.sim_state["microstep"]:
            if not 0 <= abs_position <= 24000:
                raise (
                    ValueError(
                        "`abs_position` must be between 0 and 24000 when operating in microstep mode"
                    )
                )
        else:
            if not 0 <= abs_position <= 3000:
                raise (
                    ValueError(
                        "`abs_position` must be between 0 and 3000 when operating in standard mode"
                    )
                )
    ...
    def setSpeed(self, speed_code):
        ...
        if not 0 <= speed_code <= 40:
            raise (
                ValueError(
                    "`speed_code` [{0}] must be between 0 and 40".format(speed_code)
                )
            )
    ...
    def setSlope(self, slope_code, chain=False):
        ...
        if not 1 <= slope_code <= 20:
            raise (
                ValueError(
                    "`slope_code` [{0}] must be between 1 and 20".format(slope_code)
                )
            )
    ...
    def repeatCmdSeq(self, num_repeats):
        ...
        if not 0 < num_repeats < 30000:
            raise (
                ValueError(
                    "`num_repeats` [{0}] must be between 0 and 30000".format(
                        num_repeats
                    )
                )
            )
    ...
    def delayExec(self, delay_ms):
        ...
        if not 0 < delay_ms < 30000:
            raise (
                ValueError(
                    "`delay` [{0}] must be between 0 and 40000 ms".format(delay_ms)
                )
            )
    ...
    def haltExec(self, input_pin=0):
        ...
        if not 0 <= input_pin < 2:
            raise (
                ValueError(
                    "`input_pin` [{0}] must be between 0 and 2".format(input_sig)
                )
            )
    ...
    def _calcPlungerMoveTime(self, move_steps):
        ...
        if cutoff_speed < theo_top_speed < top_speed:
            move_t = (1 / slope) * (2.0 * theo_top_speed - start_speed - cutoff_speed)
        ...
        # ramp down)
            ...
            ramp_up_halfsteps = (top_speed**2.0 - start_speed**2.0) / (2.0 * slope)
            ramp_down_halfsteps = (top_speed**2.0 - cutoff_speed**2.0) / (2.0 * slope)
            if (ramp_up_halfsteps + ramp_down_halfsteps) < (2.0 * top_speed):
                ...
                constant_halfsteps = (
                    2.0 * move_steps - ramp_up_halfsteps - ramp_down_halfsteps
                )
